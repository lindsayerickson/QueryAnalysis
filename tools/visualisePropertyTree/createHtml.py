...
parser = argparse.ArgumentParser(
    description="Generates based on the content of the ranking.tsv file a "
    + "JavaScript Json Object which contains detailed usage information "
    + "about each property")
...
sparql.setQuery("""
                #Tool: jgonsior-tree
                SELECT ?property ?propertyLabel ?subclass0 ?subclass0Label
                ?subclass1 ?subclass1Label  ?subclass2 ?subclass2Label
                ?subclass3 ?subclass3Label  ?subclass4 ?subclass4Label
                ?subclass5 ?subclass5Label  ?subclass6 ?subclass6Label
                ?subclass7 ?subclass7Label  ?subclass8 ?subclass8Label
                ?subclass9 ?subclass9Label
                WHERE {{ BIND (wd:Q18616576 as ?property)
                ?subclass0 wdt:P279 ?property .
                ...
                OPTIONAL {?subclass9 wdt:P279 ?subclass8 .}}}}}}}}}}
...
for property in sparqlResult['results']['bindings']:
    parentProperty = searchPropertyInTree(rootProperty,
                                          property['property']['value'])
    if parentProperty is None:
        parentProperty = createProperty(property['propertyLabel']['value'],
                                        property['property']['value'])
        rootProperty['children'].append(parentProperty)

    for i in range(0, 9):
        if 'subclass' + str(i) in property:
            childProperty = searchPropertyInTree(
                parentProperty,
                property['subclass' + str(i)]['value'])
            if childProperty is None:
                childProperty = createProperty(
                    property['subclass' + str(i) + "Label"]['value'],
                    property['subclass' + str(i)]['value'])
...
def createTr(property, parent, parentPrefix):
    ...
    html += "<tr data-tt-id=\"" + parentPrefix + property['qid'] \
            + "\" data-tt-parent-id=\"" + parentPrefix + "\">"
    html += "<td class=\"property\">" + property['name'] + "</td>"
    html += "<td class=\"qid\" >" + property['qid'][31:] + "</td>"
    html += "<td class=\"percentage\">" + str(property['countUserQueries']) \
            + "</td>"

    html += "</tr>\n"
    for child in sorted(property['children'], reverse=True,
                        key=cmp_to_key(compareProperties)):
